#!/usr/bin/env bash
# =============================================================================
# stage-history-slider-httpie.sh — Analyze "History / Time Travel" staging (httpie)
#
...
# past-vs-current comparison, plus one scenario forked from an old commit.
# Prints the workspace URL and a click-path per test at the end.
#
...
#
# Usage:  ./examples/stage-history-slider-httpie.sh [base_url]
...
head_hash() {
  http --ignore-stdin GET "$BASE/w/$WS_KEY/risk-trees/$TREE/history?n=50" "X-Branch:$1" \
    | jq -r '.entries[-1].commitHash // empty'
}

# Read a node id by name from the current structure on a branch.
node_id() {
  http --ignore-stdin GET "$BASE/w/$WS_KEY/risk-trees/$TREE/structure" "X-Branch:$1" \
    | jq -r --arg n "$2" '.nodes[] | (.RiskLeaf // .RiskPortfolio // .) | select(.name==$n) | .id' | head -1
}

header "Demo: Analyze History / Time Travel (httpie)"
info "Server: $BASE"
...
# Full-replacement PUT on a branch. Every existing node is re-sent as raw-JSON
# fields (:=); anything in $NEW_LEAVES is created. $1 branch, $2 label.
put_tree() {
  ...
  if http --ignore-stdin --check-status PUT "$BASE/w/$WS_KEY/risk-trees/$TREE" "X-Branch:$branch" \
       name="Cyber Risk Register" \
       portfolios:="[ {\"id\": \"$ROOT_ID\", \"name\": \"Root\", \"parentName\": null} ]" \
       leaves:="$leaves" \
       newPortfolios:='[]' \
       newLeaves:="[ ${NEW_LEAVES:-} ]" >/dev/null 2>&1; then
    ok "$label"
  else
    fail "$label — non-2xx from server"; exit 1
  fi
}
...
if http --ignore-stdin --check-status POST "$BASE/w/$WS_KEY/risk-trees/$TREE/revert" "X-Branch:main" \
     toCommit="$REVERT_TARGET" >/dev/null 2>&1; then
  ok "Reverted to C4 as a forward commit"
else
  fail "Revert failed (non-2xx)"; exit 1
fi

# ── Step 7: Scenarios ────────────────────────────────────────────────────────
header "Step 7 — Scenario \"mitigated\" (forked from main) + edits M1, M2"
SC=$(http --ignore-stdin POST "$BASE/w/$WS_KEY/scenarios" \
  name="mitigated" \
...
SCH=$(http --ignore-stdin POST "$BASE/w/$WS_KEY/scenarios" \
  name="pre-insider-audit" \
  source:="{\"type\": \"commit\", \"hash\": \"$C2_HASH\"}")
