...
Simulations are **reproducible by construction**. Randomness comes from a counter-based PRNG (HDR — Hubbard, Winter Simulation Conference 2019): every leaf carries a stored `seedVarId` assigned at creation, every workspace a stored `seedEntityId`, and each trial's draws are pure functions of those identities. Re-running a saved tree reproduces its figures exactly; renaming nodes changes nothing; editing one leaf leaves every other leaf's draws byte-identical (common random numbers), so scenario differences are attributable solely to the edit. Exports carry the seed identities, so figures can be reproduced across deployments by re-importing them.
