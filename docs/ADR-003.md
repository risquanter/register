...
> **Note:** `TreeProvenance` exists in code but is unused (dead code). `RiskTreeWithLEC` has been removed; provenance is embedded in `RiskResult.provenances`. Future cleanup should remove the `TreeProvenance` case class.
...
- `TreeProvenance` case class in `Provenance.scala` — defined and has codecs, but zero production usage (only referenced in `ProvenanceSpec` serialization test)
