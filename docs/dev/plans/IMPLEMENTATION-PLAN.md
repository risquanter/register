...
**Reference:** Design analysis in `docs/archive/plans/milestone-2b-cache-and-decisions.md`
  ...
  def connect(treeId: String): EventStream[SSEEvent] =
    EventStream.fromCustomSource[SSEEvent](
      start = (fireEvent, _, _, _) =>
        val source = new EventSource(s"/w/${workspaceKey.reveal}/events/tree/$treeId")
