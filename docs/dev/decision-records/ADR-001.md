...
final case class RiskResultCache(
  cache: Map[String, RiskResult]  // Raw String for domain ID
)

// ✅ GOOD: Use NodeId nominal wrapper consistently (ADR-018)
final case class RiskResultCache(
  cache: Map[NodeId, RiskResult]  // NodeId wraps SafeId.SafeId
)
...
// ✅ GOOD: TreeIndex with NodeId (nominal case class wrapping SafeId.SafeId)
final case class TreeIndex(
...
.in("risk-trees" / path[TreeId]("treeId"))

// Controller - no parsing, just wire
val getById = getByIdEndpoint.serverLogicSuccess { id =>
  riskTreeService.getById(id)  // id already validated TreeId
}
...
private val db = TrieMap[TreeId, RiskTree]()
```

**Rationale:**
1. **Type Alignment**: Domain model (`RiskTree.id: TreeId`) matches all layers
2. **Single Source of Truth**: Change ID type once, codec updates automatically
3. **Future-Proof**: ID type changes require only type + codec update, not all signatures
...
### 8. External-System Output Boundary Constraints

Some values are not user input — they are config-loaded credentials or tokens that are
sent **verbatim** to external services as HTTP header field values (e.g., `Authorization:
Bearer <token>`). These values require a different Iron constraint than values stored in
the database or displayed in the UI.
...
control characters — preventing the value from splitting an HTTP response or injecting
additional headers at the transport layer.
...
(redacted `toString`, explicit `reveal`, no `copy`/`unapply`, manual `equals`/`hashCode`).
Use `ValidationUtil.refineExternalToken` in the `fromString` constructor. Do not add
a JSON codec — config-only types are never serialised to or from JSON.
...
- Injection guard is at the **type boundary** (config load), not the HTTP call site.
  Once a value is an `ExternalTokenStr`, it is statically known to be safe for headers.
...
- Space (`0x20`) is excluded: HTTP/1.1 header fields use SP as a list separator.
  No realistic token format requires a literal space character.

#### Cross-references

- **ADR-022**: credential class design (R1–R8) — `final class`, `private val`, `reveal`,
  redacted `toString`, no JSON codec for config-only types
- **ADR-001 §5**: URL refinement for internal service endpoints (`UrlConstraint`, http/https)
- `ValidationUtil.refineExternalToken` / `ValidationUtil.refineSecureUrl` — validation helpers
...
val getById = getByIdEndpoint.serverLogicSuccess { id =>
  riskTreeService.getById(id)  // Already validated TreeId
}
```

**Why this matters:**
- Non-valid input (e.g., `/risk-trees/abc`) should return 400 Bad Request from Tapir, not 500 from controller
- Domain identifier type (`TreeId`) should flow through all layers for consistency
- ID type changes become trivial: change type + codec, not all controllers/services
...
| `RiskTreeEndpoints` | Uses Iron types (including `TreeId` for tree IDs, `NodeId` for node IDs) |
| `RiskTreeController` | Wires only—no validation or parsing logic |
| `RiskTreeService` | Iron types in signatures (including `TreeId` for tree IDs) |
