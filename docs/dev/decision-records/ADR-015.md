...
**Status:** Accepted  
**Date:** 2026-02-09  
**Tags:** caching, simulation, architecture, domain-model, irmin  
**Supersedes:** Parts of ADR-014 (refines cache wiring details)

> **2026-07-18 (milestone 2b Phase A):** the resolver pattern (`ensureCached`
> as the single simulation entry point) stands, but the storage layer this
> ADR sketches (`TreeCacheManager`/`RiskResultCache`, NodeId keys, ancestor-
> path invalidation) is retired. The cache is now content-addressed
> (`ContentCacheRegistry`/`ContentCache`, `ContentHash` keys, leaf entries only) and
> has **no invalidation** — an edited leaf hashes to a new key and misses.
> See `docs/archive/plans/milestone-2b-cache-and-decisions.md` (DD-14…DD-20).

---

## Context

- Tree **structure** (node definitions, topology) and simulation **results** are separate concerns with different lifecycles — conflating them produces unnecessary recomputation when only one side changes
- The cache must be the **single source of truth** for simulation outputs; query paths that bypass the cache create consistency gaps and redundant Monte Carlo work
- A `CachedResultResolver` abstraction — `ensureCached(nodeId)` — decouples cache-miss handling from the cache's storage contract, allowing either one to evolve independently
- All higher-level query operations (LEC curves, exceedance probabilities, provenance) reduce to `map` compositions over `ensureCached`, eliminating duplicated simulation triggers
- Cache invalidation must propagate up the ancestor path: a node result depends on all its descendants, so any structural or distributional change must evict the affected node and every ancestor
...
### Core Abstraction: CachedResultResolver
...
Irmin (persistent)     ←→     RiskTreeRepository
    ...
    RiskNode (tree definition)
        ↓
    CachedResultResolver.ensureCached(nodeId)
        ↓
...
### CachedResultResolver Service (New)
...
class CachedResultResolverLive(
...
def probOfExceedance(nodeId: NodeId, threshold: Loss): Task[Double] =
...
```scala
def simulateSubtree(nodeId: NodeId): Task[LossDistribution] =
  for
    ...
    result <- node match
      ...
      case portfolio: RiskPortfolio =>
        // Recurse to children in parallel (licensed by TrialOutcomes laws, ADR-009)
        for
          childResults <- ZIO.foreachPar(portfolio.children)(child =>
            cache.get(child.id).flatMap {
              case Some(r) => ZIO.succeed(r)  // reuse cached
              case None    => simulateSubtree(child.id)  // recurse
            }
          )
          combined = RiskResultGroup(nodeId, childResults*)  // aggregate, children retained
...
// GOOD: Delegate to CachedResultResolver, query is a pure transform
def getLEC(nodeId: NodeId) =
...
cache["ops-risk"]    = RiskResultGroup(outcomes=...)  // aggregated cyber+hardware
cache["market-risk"] = RiskResult(outcomes=...)
cache["portfolio"]   = RiskResultGroup(outcomes=...)  // aggregated all
```
...
2. For `ops-risk`: cache miss → recurse
   ...
   - Aggregate → `cache.put("ops-risk", RiskResultGroup(...))`
3. For `market-risk`: cache **HIT** → reuse
4. Aggregate `[ops-risk-result, market-risk-result]`
5. `cache.put("portfolio", RiskResultGroup(...))`

**What got recomputed:**
- `ops-risk` aggregation (not simulation, just the TrialOutcomes fold)
...
RiskNode (tree definition)
    ↓
Simulation (in-memory, via CachedResultResolver)
    ↓
...
| Phase | Status |
|-------|--------|
| `CachedResultResolver` service | ✅ Implemented |
...
2. **lookupNodeDefinition implementation?**  
   ...
   provides O(1) lookup. `CachedResultResolver` methods take `RiskTree` parameter.
...
- **ADR-014:** Simulation Result Caching and Render-Time Curve Generation (parent ADR)
