...
## Exceptions: Acceptable Uses of `require()` and Thrown Exceptions

While errors should be values, the following patterns are acceptable:
...
### 4. Throwing APIs Outside Our Control — Conversion Layer

Some required operations signal failure only by throwing, and we do not own
their signatures: JVM built-ins (`Math.addExact`), third-party parsers,
checked-arithmetic intrinsics. Wrap such a call at the nearest owned
construction boundary and convert the exception to a typed value there,
so no exception ever crosses a public API:

```scala
// Math.addExact is the only checked Long addition the JVM offers; it throws
// ArithmeticException on overflow. The smart constructor is the conversion
// layer: callers only ever see Validation.
def create(nodeId: NodeId, results: LossDistribution*)(using SimulationConfig)
    : Validation[ValidationError, RiskResultGroup] =
  try Validation.succeed(RiskResultGroup(nodeId, results*))
  catch {
    case _: ArithmeticException =>
      Validation.fail(ValidationError(
        s"riskPortfolio.$nodeId",
        ValidationErrorCode.CONSTRAINT_VIOLATION,
        ValidationMessages.aggregatedLossOverflow
      ))
  }
```

**When acceptable:**
- ✅ The throwing API is external (JVM, library) — its signature is not ours to change
- ✅ The catch names the narrowest exception type (never `NonFatal` or `Throwable`)
- ✅ Conversion happens in the innermost method we own; the public signature is value-typed
- ✅ The wrapped failure is reachable from user data — unreachable-by-construction failures stay `require` safety nets (see 1–3)
...
| `domain/data/LossDistribution.scala` (`RiskResultGroup.create`) | Conversion layer over a throwing JVM API |
