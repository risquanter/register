...
val result = RiskResult(leaf.id, trials, List(provenance))
cache.put(leaf.id, result)  // provenances always populated before cache write

// aggregation — read-time derivation, nothing merged or stored on the aggregate
RiskResultGroup(portfolio.id, children*).provenances  // == children.flatMap(_.provenances)
```
...
- `RiskResultGroup.provenances` derives child provenances at read time: `children.flatMap(_.provenances)`
- Portfolio nodes expose child provenances; no additional provenance is added or stored on the aggregate
- Leaf nodes carry their single provenance in `RiskResult.provenances`
...
- Provenance is embedded as `List[NodeProvenance]` in `RiskResult` (leaves); aggregates derive it at read time via `RiskResultGroup.provenances` (see ADR-009)
