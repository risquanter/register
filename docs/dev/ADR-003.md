...
Portfolio results accumulate leaf provenances bottom-up through `RiskResult.combine`; a portfolio
node's `provenances` list is the union of all leaf provenances in its subtree. Provenance data is
surfaced through a dedicated audit endpoint, not embedded in curve or exceedance responses.
...
// combine — bottom-up accumulation
RiskResult.combine(a, b)  // a.provenances ++ b.provenances
```
...
- `RiskResult.combine` accumulates provenances automatically during aggregation
