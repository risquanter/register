...
See **[ADR-015-proposal.md](./ADR-015-proposal.md)** for the service layer that builds on this cache:

- **`RiskResultResolver`**: Service with `ensureCached(nodeId)` primitive
- **Query APIs as compositions**: `resolver.ensureCached(nodeId).map(transform)`
- **Eliminates `RiskTreeResult`**: Cache becomes the source of truth for results
...
- [ADR-015-proposal.md](./ADR-015-proposal.md) — Cache Integration: RiskResultResolver pattern
