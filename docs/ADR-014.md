...
def onNodeChanged(treeId: Treancestor path — O(depth)
...
def getLEC(nodeId: NodeId) = simulateSueId): UIO[Unit] =
  cacheManager.onTreeStructureChanged(treeId)  // O(n) — clears everything

// GOOD: Invalidate only the btree(nodeId).map(generateCurvePoints)
