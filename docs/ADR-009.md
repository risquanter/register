...
- Aggregate nodes must produce loss distributions for LEC visualisation even though only leaf nodes carry distribution parameters — without a uniform result type, every consumer must branch on node kind
- Results must be **compositional**: combining two `RiskResult` values must yield a value indistinguishable in shape from a natively simulated one, enabling arbitrary subtree aggregation
- An aggregate over an empty child set requires a **lawful identity element** so that fold operations remain total and respect the Monoid laws
- The identity element's `nTrials` is derived from configuration rather than being a universal constant — any identity instance must be scoped to `SimulationConfig` to avoid trial-count drift
- Provenance must be preserved through aggregation so that any node's LEC curve can be traced back to the contributing leaves
...
- Both have `probOfExceedance(threshold: Loss): Double`
...
See **[ADR-015.md](./ADR-015.md)** for the pattern:
...
- ADR-015: Cache Integration - RiskResultResolver pattern
