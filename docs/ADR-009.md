...
| `RiskResultResolverLive` | `services/cache/RiskResultResolverLive.scala` | Cache-aware simulation (replaces `RiskTreeResult`) |
...
### ✅ RiskTreeResult Removed

`RiskTreeResult` has been removed. The new architecture caches `RiskResult` per node in `RiskResultCache`, keyed by `NodeId`. Tree structure for invalidation is provided by `TreeIndex`.
