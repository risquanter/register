...
- We deliberately **do not expose a global `Identity[RiskResult]` instance**; production code uses non-empty folds with the total `combine`.
- If a neutral element is needed (e.g., in tests), construct it explicitly for the configured `nTrials` as a zero-loss result.

**Future-proofing:** If a typeclass shape like `Identity[RiskResult]` is ever required, prefer a context-dependent instance (e.g., `given Identity[RiskResult](using SimulationConfig)`) over phantom type encodings. This threads the configured `nTrials` without type-level gymnastics, but still limits usage to scopes where configuration is available.
