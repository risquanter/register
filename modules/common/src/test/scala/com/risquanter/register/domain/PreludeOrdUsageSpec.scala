  ...
  def spec = suite("PreludeOrdUsageSpec")(
    ...
    suite("RiskResult - Ord[Loss] with TreeMap")(
      test("maxLoss uses Ord[Loss] - single loss") {
        val result = RiskResult(SafeId.SafeId("test".refineUnsafe), Map(1 -> 5000L), 10)
        
        assertTrue(result.maxLoss == 5000L)
      },
      
      test("maxLoss uses Ord[Loss] - multiple losses") {
        val result = RiskResult(SafeId.SafeId("test".refineUnsafe), Map(1 -> 1000L, 2 -> 5000L, 3 -> 2000L), 10)
      ...
      test("empty result has zero max/min") {
        val result = RiskResult(SafeId.SafeId("empty".refineUnsafe), Map.empty, 10)
      ...
      test("outcomeCount is sorted by Loss (ascending)") {
        val result = RiskResult(SafeId.SafeId("test".refineUnsafe), Map(
          1 -> 3000L,
          2 -> 1000L,
          3 -> 5000L,
          4 -> 2000L
    ...
    suite("RiskResultGroup - Ord[Loss] with TreeMap")(
      test("maxLoss uses Ord[Loss] for aggregated results") {
        val child1 = RiskResult(SafeId.SafeId("r1".refineUnsafe), Map(1 -> 1000L, 2 -> 2000L), 10)
        val child2 = RiskResult(SafeId.SafeId("r2".refineUnsafe), Map(1 -> 3000L, 2 -> 4000L), 10)
        
        val group = RiskResultGroup(SafeId.SafeId("total".refineUnsafe), 10, child1, child2)
      ...
      test("outcomeCount sorted with aggregated losses") {
        val child1 = RiskResult(SafeId.SafeId("r1".refineUnsafe), Map(1 -> 1000L, 2 -> 2000L), 10)
        val child2 = RiskResult(SafeId.SafeId("r2".refineUnsafe), Map(1 -> 500L, 2 -> 3000L), 10)
      ...
      test("empty group has zero max/min") {
        val group = RiskResultGroup(SafeId.SafeId("empty".refineUnsafe), 10)
